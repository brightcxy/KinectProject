...
Platform boxes[] = new Platform[4000];

float zPos;
int counter = 0;
float minthreshold = 0;
float maxthreshold = 4500;
...
void draw(){
  background(0);
  spotLight(255, 255, 255, 480/2, 960/2, 2400, 0, 0, -1, PI/4, 2);
...
class Platform {
  ...
  void position(float tx, float ty) {
    ...
    y = ty;
    z = -100;
    index = int(map(x,0,2048,0,512)) + int(512*(map(y,0,1272,0,424)-1));
    an = 0;
  }
